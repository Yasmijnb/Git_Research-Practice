...
autoplot(AGE.pca, data = AGE, colour = 'network', main = 'Age')

# Make a PCA for sex
SEX <- rbind(men, women)
SEX$network <- c(rep('men', nrow(men)), rep('women', nrow(women)))
SEX.pca <- prcomp(SEX[,c(2:7, 12:13, 16, 20:21)], scale = T)
autoplot(SEX.pca, data = SEX, colour = 'network', main = 'Sex')
...
autoplot(MEN.pca, data = MEN, colour = 'network', main = 'Men')
...
autoplot(WOMEN.pca, data = WOMEN, colour = 'network', main = 'Women')
