diffcon_plot <- function(res_path){
  ...
  fa_labels <- read.table('Lipid_groups.txt', check.names = F, header = T, sep='\t', row.names = 1)
  ...
  for (file in files){
    ...
    myplot <- ggplot(diffcon_data, aes(x = data_diff_conn, y = log10_pvalues, color = group)) +
      geom_point(aes(shape = group, color = group), size=6) + 
      ...
      scale_shape_manual(name = 'Lipid Type', values = c(15:21)) + scale_color_manual(name = 'Lipid Type', values=c("#0073C2", "#EFC000", "#868686", "#CD534C", "#7AA6DC", "#003C6799", "#8F7700")) +
