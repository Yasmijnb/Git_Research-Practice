...
young.women.corr <- PCLRC.gmm(young.women[,23:43])$CorrMat
young.women.corr <- young.women.corr[upper.tri(young.women.corr, diag = FALSE)]
young.women.75 <- quantile(abs(young.women.corr), probs = 0.75)

old.women.corr <- PCLRC.gmm(old.women[,23:43])$CorrMat
old.women.corr <- old.women.corr[upper.tri(old.women.corr, diag = FALSE)]
old.women.75 <- quantile(abs(old.women.corr), probs = 0.75)

threshold <- min(young.women.75, old.women.75)
...
saveRDS(women.pclrc, 'Results/women.pclrc.rds')

# Save as xlsx for COVSCA
write.xlsx(women.pclrc$AdjMat1, 'COVSCA/Adjacency_matrix_youngwomen.xlsx')
write.xlsx(women.pclrc$AdjMat2, 'COVSCA/Adjacency_matrix_oldwomen.xlsx')
...
# young_adj <- as.data.frame(women.pclrc$AdjMat1)
# old_adj <- as.data.frame(women.pclrc$AdjMat2)
young_adj <- read.csv('Results/Adjacency_matrix_youngwomen.csv')
young_adj <- young_adj[,-1]
old_adj <- read.csv('Results/Adjacency_matrix_oldwomen.csv')
old_adj <- old_adj[,-1]
...
# write.csv(young_adj, file = 'Adjacency_matrix_youngwomen.csv')
# write.csv(old_adj, file = 'Adjacency_matrix_oldwomen.csv')
...
pvalues <- as.data.frame(women.pclrc$Pval)
...
pvalues$diffcon <- women.pclrc$Diff_Conn
...
for (i in 1:7) {
  g <- ggplot(data = pvalues, aes_string(x = nm[3], y = nm[2], fill = nm[5+i])) + 
    ...
    # Add a title
    ggtitle(paste('Young women vs Old women\nMT =', testing[i], '\np-value threshold =', thresholds[i])) +
    # Use nicer colours
  ...
  print(g)
  ggsave(filename = paste0('PCLRC_Women_',testing[i], '_p-threshold=', thresholds[i], '.png'), 
         plot = g)
}
